...
## Watchlist deletion and community ranking

Owners can delete a product from a watchlist card or its detail page after a confirmation dialog. The authenticated, same-origin DELETE endpoint scopes deletion to the owner; foreign IDs return 404. Database cascades remove its price history, analyses, alternatives, public find and reactions. Deletion is permanent and ends future tracking.

Signed-in accounts can like or dislike a published find, switch their reaction, or click the selected reaction to remove it. A database uniqueness constraint permits one reaction per account/find; requests set an explicit state so retries cannot add extra votes. Reactions are rate limited and unavailable on unpublished/deleted finds. Visitors see counts with a sign-in link.
