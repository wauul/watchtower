...
export async function DELETE(req:Request,{params}:{params:{id:string}}){if(!sameOrigin(req))return new Response('Forbidden',{status:403});const email=sessionEmail();if(!email)return new Response('Unauthorized',{status:401});try{const result=await db.product.deleteMany({where:{id:params.id,email}});return result.count?Response.json({ok:true},{headers:privateHeaders}):new Response('Not found',{status:404});}catch(e){return failure(e);}}
