...
export default function Layout({children}:{children:React.ReactNode}){return <html lang="en"><body><header className="nav"><Link href="/" className="brand"><span className="brand-icon"><Radar size={23}/></span>watchtower<span className="beta">BETA</span></Link><nav><Link href="/">Track a product</Link><Link className="nav-dashboard" href="/dashboard">My watchlist <ArrowUpRight size={16}/></Link></nav></header>{children}<footer><Link href="/" className="brand"><Radar size={19}/> watchtower</Link><span>A little patience. A better price.</span><span>Built for thoughtful buying.</span></footer></body></html>;}
