...
export async function checkProduct(id:string){
 ...
 await db.product.update({where:{id},data:{stock:current.stock,lastCheckedAt:new Date(),lastError:null,history:{create:{price:current.price,stock:current.stock}}}});
 ...
 try{options=await alternatives(product.name,product.country,product.currency);if(!process.env.GOOGLE_CSE_API_KEY)searchNote='Alternative search is not configured.';}catch{searchNote='Alternative search was unavailable for this check.';}
